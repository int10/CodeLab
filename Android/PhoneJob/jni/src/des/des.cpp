...
void * des_memcpy(void * dest,const void * src, int count)
{
	char * tmp = (char *)dest; const char * s = (char*)src;
	while(count--)
	{
		*tmp++ = *s++ ;
	}
                      ...
                      int len )
{
    ...
    while( len > 0 )
    {
        des_memcpy( temp, input, 8 );
        des_crypt( ctx->dsk, input, output );

        for( i = 0; i < 8; i++ )
            output[i] = output[i] ^ iv[i];

        des_memcpy( iv, temp, 8 );
