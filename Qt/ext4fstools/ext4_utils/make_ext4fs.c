					     ...
					     struct selabel_handle *sehnd)
{
	...
	*dentries.inode = inode;
	...
	if (sehnd) 
	{
		char *path = NULL;
		char *secontext = NULL;
		char *reg_free_path = NULL;
		
		char* sepath = NULL;
		
		asprintf(&path, "%slost+found", dir_path);
		
		//fprintf(stderr, "build_default_directory_structure - before : str_replace path=%s", path);
		
		//reg_free_path = str_replace(path, "", char *with);
		
		//fprintf(stderr, "build_default_directory_structure - before : str_replace reg_free_path=%s", reg_free_path);

		//sepath = make_reg_free_string(path);
		
		sepath = path;
		
		path = NULL;
		
		fprintf(stderr, "build_default_directory_structure - sepath - %s\n", sepath);
		...
		struct selabel_handle *sehnd, int verbose, time_t fixed_time)
{
	...
	//fprintf(stderr, "build_directory_structure enter - full_path : %s\n", full_path);
	//fprintf(stderr, "build_directory_structure enter - dir_path : %s\n", dir_path);
	...
	for (i = 0; i < entries; i++) 
	{
		...
		//fprintf(stderr, "build_directory_structure - dir_path : %s, namelist[%d]->d_name : %s\n", dir_path,  i, namelist[i]->d_name);
		
		//fprintf(stderr, "build_directory_structure - full_path : %s, namelist[%d]->d_name : %s\n", full_path,  i, namelist[i]->d_name);
		...
		if (fs_config_func != NULL) 
		{
			...
			//fprintf(stderr, "build_directory_structure - dentries[%d].path : %s\n", i, dentries[i].path);
		...
		////fprintf(stderr, "build_directory_structure - if (sehnd) - dentries[%d].path : %s\n", 0, dentries[0].path);
		...
		if (sehnd) 
		{
			...
			//sepath = make_reg_free_string(dentries[i].path);
			
			sepath = dentries[i].path;
			
			////fprintf(stderr, "build_directory_structure - 1 - sepath - %s\n", sepath);
		
			if (selabel_lookup(sehnd, &dentries[i].secon, sepath, stat.st_mode) < 0) 
			{
				error("208 cannot lookup security context for %s", sepath);
			}
		...
		////fprintf(stderr, "build_directory_structure - #endif - dentries[%d].path : %s\n", 0, \
				dentries[0].path);
		
#endif

		////fprintf(stderr, "build_directory_structure - if (S_ISREG(stat.st_mode)) - dentries[%d].path : %s\n", 0, \
				dentries[0].path);
		...
		if (S_ISREG(stat.st_mode)) {
			...
			/*
			fprintf(stderr, 
				"build_directory_structure - readlink - full_path : %s, link : %s", 
		...
		////fprintf(stderr, "build_directory_structure - namelist - dentries[%d].path : %s\n", 0, \
				dentries[0].path);
		
	}
	free(namelist);
	
	////fprintf(stderr, "build_directory_structure - needs_lost_and_found - dentries[%d].path : %s\n", 0, \
				dentries[0].path);
	...
	if (needs_lost_and_found) 
	{
		...
		if (sehnd) 
		{
			...
			//sepath = make_reg_free_string(dentries[0].path);
			
			sepath = dentries[0].path;
			
			////fprintf(stderr, "build_directory_structure - sepath - 2 - %s\n", sepath);
			
			if (selabel_lookup(sehnd, &dentries[0].secon, sepath, dentries[0].mode) < 0)
			{
				error("258 cannot lookup security context for %s", sepath);
			}
		}
	...
	////fprintf(stderr, "build_directory_structure - make_directory - dentries[%d].path : %s\n", 0, \
				dentries[0].path);
	...
	for (i = 0; i < entries; i++) 
	{
		...
		else if (dentries[i].file_type == EXT4_FT_DIR) 
		{
			...
			//fprintf(stderr, \
				"build_directory_structure - dentries[%d].path : %s\n", \
				i, \
				dentries[i].path);
			...
			//fprintf(stderr, \
				"build_directory_structure - subdir_full_path: %s, subdir_dir_path: %s\n", \
				subdir_full_path, \
				subdir_dir_path);
						 ...
						 FILE* block_list_file)
{
	...
	if (sehnd) 
	{
		...
		sepath = make_reg_free_string(mountpoint);
		
		
		//sepath = _mountpoint;
		
		//fprintf(stderr, "make_ext4fs_internal - sepath - %s\n", sepath);
	...
	//fprintf(stderr, "make_ext4fs_internal - exit normal.\n");
