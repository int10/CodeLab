...
int test_reg()
{
	...
	//regerr = regcomp(&re, "^/build\\.prop$", REG_EXTENDED | REG_NOSUB);
	
	char* reg_str = "^/build\\.$";
	
	char* src_str = "/build.";
	
	regerr = regcomp(&re, reg_str, REG_EXTENDED | REG_NOSUB);

	if(regerr)
	{
		fprintf(stderr, "test_reg - regcomp error : %s\n", reg_str);
	}
	
	//rc = regexec(&re, "/build\\.prop", 0, 0, 0);
	
	rc = regexec(&re, src_str, 0, 0, 0);
	
	if(rc)
	{
		fprintf(stderr, "test_reg - regexec error reg_str : |%s| src_str : |%s|\n", reg_str, src_str);
	}
...
int main(int argc, char **argv)
{
	...
	fprintf(stderr, "version: 1.3.20151215\n");

	//fprintf(stderr, "\nmain - before OPTS\n");
	
	//fprintf(stderr, "\nmain - make_reg_free_string: %s\n", make_reg_free_string("strdc++..cpp--"));
	...
	while ((opt = getopt(argc, argv, "l:j:b:g:i:I:L:a:S:T:C:B:fwzJsctv")) != -1) 
	{
		...
		case 'S':
			...
			//fprintf(stderr, "\nmain - before selabel_open\n");
			
			sehnd = selabel_open(SELABEL_CTX_FILE, seopts, 1);
			
			//fprintf(stderr, "\nmain - after selabel_open\n");
	...
	if (fs_config_file) 
	{
		//fprintf(stderr, "\nmain - before load_canned_fs_config\n");
		...
		//fprintf(stderr, "\nmain - after load_canned_fs_config\n");
	...
	//fprintf(stderr, "\nmain - before make_ext4fs_internal\n");
