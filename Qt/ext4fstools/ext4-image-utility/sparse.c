...
static int write_all_blocks(struct sparse_file *s, struct output_file *out)
{
    ...
    {
        //DEBUG
        //backed_block_debug_view("write_all_blocks", bb);

        //file_offset(out, "write_all_blocks 1");

        //fprintf(stderr, "write_all_blocks bb->block : %u", bb->block)
        ...
        //file_offset(out, "write_all_blocks 2");

		sparse_file_write_block(out, bb);
		last_block = backed_block_block(bb) +
				DIV_ROUND_UP(backed_block_len(bb), s->block_size);

		//file_offset(out, "write_all_blocks 3");
	}
