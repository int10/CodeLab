...
{
    char *newbuffer;
    int nsects = m_blocksize/sect_size;
    int ret;
    lloff_t sectno;
    ...
    if(!newbuffer)
    {
        newbuffer = new char[m_blocksize];
        ...
        ret = read_disk(m_handle, newbuffer, sectno, nsects, sect_size);
    ...
    memcpy(buffer, newbuffer, m_blocksize);
...
int Ext2Partition::mount()
{
    ...
    read_disk(m_handle, &sblock, relative_sect + 2, 2, sect_size);	/* read superBlock of root */
    ...
    m_blocksize = EXT2_BLOCK_SIZE(&sblock);
    ...
    LOG("Block size %d, inp %d, inodesize %d\n", m_blocksize, inodes_per_group, inode_size);
    ...
    if((m_blocksize/sect_size) <= 2)
        read_disk(m_handle, tmpbuf, relative_sect + ((m_blocksize/sect_size) + 2), gSizes, sect_size);
    else
        read_disk(m_handle, tmpbuf, relative_sect + (m_blocksize/sect_size), gSizes, sect_size);
...
Ext2File *Ext2Partition::read_dir(EXT2DIRENT *dirent)
{
    ...
    if(!dirent->dirbuf)
    {
        dirent->dirbuf = (EXT2_DIR_ENTRY *) new char[m_blocksize];
        if(!dirent->dirbuf)
            return NULL;
    ...
    else
    {
        ...
        if(IS_BUFFER_END(dirent->next, dirent->dirbuf, m_blocksize))
        {
            dirent->next = NULL;
...
Ext2File *Ext2Partition::read_inode(uint32_t inum)
{
    ...
    if(!inode_buffer)
    {
        inode_buffer = (char *)malloc(m_blocksize);
        if(!inode_buffer)
            return NULL;
    }
    ...
    inode_index = (index % m_blocksize);
    blknum = desc[group].bg_inode_table + (index / m_blocksize);
	...
	//std::cerr<<"Ext2Partition::read_inode - sizeof(file->inode)"<<sizeof(file->inode)<<std::endl;

	//std::cerr<<"Ext2Partition::read_inode - m_blocksize"<<m_blocksize<<std::endl;

	//hack for things after inode
	//modify 20151022
	//memcpy(file->block, src, m_blocksize > 4096 ? 4096 : m_blocksize);
	memcpy(file->block, src, m_blocksize-inode_index > 4096 ? 4096 : m_blocksize-inode_index);
	//file->inode = *src;
...
lloff_t Ext2Partition::extent_binarysearch(EXT4_EXTENT_HEADER *header, lloff_t lbn, bool isallocated)
{
    ...
    for(int i = 0; i < header->eh_entries; i++)
    {
        ...
        {
            child = (EXT4_EXTENT_HEADER *) new char [m_blocksize];
...
uint32_t Ext2Partition::fileblock_to_logical(EXT2_INODE *ino, uint32_t lbn)
{
    ...
    sz = m_blocksize / sizeof(uint32_t);
