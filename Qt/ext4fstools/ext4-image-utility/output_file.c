...
static int file_skip(struct output_file *out, int64_t cnt)
{
	...
	//file_offset(out, "file_skip 1");
	
    ret = _lseeki64(outn->fd, cnt, SEEK_CUR);
	//ret = _lseek(outn->fd, cnt, SEEK_CUR);

	//file_offset(out, "file_skip 2");
...
static int get_5836800 = 0;
static int jsq_5836800 = 0;

int64_t file_offset(struct output_file *out, const char* msg)
{
	struct output_file_normal *outn = to_output_file_normal(out);

	int64_t offset = 0;

    offset = _lseeki64(outn->fd, 0, SEEK_CUR);

    fprintf(stderr, "%s offset : %lld, offset in block: %lld\n", msg, offset, offset/4096);
	...
	if(5836800 == offset)
	{
		...
		if(jsq_5836800 == 31)
		{
			fprintf(stderr, "file_offset - jsq_5836800 : %d\n", jsq_5836800);	
		}
		fprintf(stderr, "file_offset - jsq_5836800 : %d\n", jsq_5836800++);	
	}

	if(1 == get_5836800 && 4096 == offset)
	{
		//fprintf(stderr, "file_offset - jsq_5836800 : %d\n", jsq_5836800);	
	}
...
static int file_write(struct output_file *out, void *data, int len)
{
	...
	//if(offset == 5832704)
	{
		//fprintf(stderr, "5832704 found!\n");
	}

    if(offset == 0x1000)
    {
       // fprintf(stderr, "0x1000 found!\n");
    }

    ret = _write(outn->fd, data, len);

	//fprintf(stderr, "file_write offset :  %lld, len : %d, ret : %d\n", offset, len, ret);

	if (ret < 0) {
		error_errno("write");
		
		//fprintf(stderr, "file_write - write");
	...
	else if (ret < len) 
	{
		...
		//fprintf(stderr, "file_write - incomplete write");
...
static int callback_file_open(
        //struct output_file *out __unused,
        struct output_file *out ,
        //int fd __unused
        int fd
        )
...
static int write_sparse_skip_chunk(struct output_file *out, int64_t skip_len)
{
    ...
    {
        //error("don't care size %PRIi64" is not a multiple of the block size %u", skip_len, out->block_size);

        error("don't care size %lld is not a multiple of the block size %u", skip_len, out->block_size);
		...
		void *data)
{
    ...
    //offset = _lseek(out->, 0, SEEK_CUR);

	//fprintf(stderr, "\write_normal_data_chunk - offset 1 %lld\n", offset);
	...
	//fprintf(stderr, "\write_normal_data_chunk - offset 2 %lld\n", offset);
...
#ifndef USE_MINGW
    data = mmap(NULL, buffer_size, PROT_READ, MAP_SHARED, fd,
			aligned_offset);
	if (data == MAP_FAILED) {
		return -errno;
	}
