...
static int sparse_file_read_sparse(struct sparse_file *s, int fd, bool crc)
{
    ...
    //fprintf(stderr, "\nsparse_file_read_sparse - offset enter : %lld\n", offset);

    //fprintf(stderr, "\nsparse_file_read_sparse - offset enter : %" PRId64 "\n", offset);
    ...
    //fprintf(stderr, "\nsparse_file_read_sparse - after read_all : %lld\n", offset);
    ...
    //fprintf(stderr, "\nsparse_file_read_sparse - offset before for %lld\n", offset);
	...
	for (i = 0; i < sparse_header.total_chunks; i++) {
		...
		if (sparse_header.chunk_hdr_sz > CHUNK_HEADER_LEN) {
			 ...
			 * we expected.
			 */
            _lseek(fd, sparse_header.chunk_hdr_sz - CHUNK_HEADER_LEN, SEEK_CUR);
		}

        offset = _lseeki64(fd, 0, SEEK_CUR);

        //fprintf(stderr, "sparse_file_read_sparse - offset bf %lld\n", offset);
        ...
        //fprintf(stderr, "sparse_file_read_sparse - offset af %lld\n", offset);
...
struct sparse_file *sparse_file_import(int fd, bool verbose, bool crc)
{
    ...
    //fprintf(stderr, "sparse_file_import - offset 1 %lld\n", offset);
